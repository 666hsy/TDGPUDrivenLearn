    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                float texNei = _AlphaMapSize.y;		// 1/width
                ...
                int and5 = (1 << 5) - 1;
                int2 blend1 = int2(blendData1 >> 11, (blendData1 >> 6) & and5);		//主纹理索引+次纹理索引
                ...
                float w3 = ((uv1.y - uv2.y) * IN.uv.x + (uv2.x - uv1.x) * IN.uv.y + uv1.x * uv2.y - uv2.x * uv1.y) / ((
                    uv1.y - uv2.y) * uv3.x + (uv2.x - uv1.x) * uv3.y + uv1.x * uv2.y - uv2.x * uv1.y);
                float w2 = ((uv1.y - uv3.y) * IN.uv.x + (uv3.x - uv1.x) * IN.uv.y + uv1.x * uv3.y - uv3.x * uv1.y) / ((
                    uv1.y - uv3.y) * uv2.x + (uv3.x - uv1.x) * uv2.y + uv1.x * uv3.y - uv3.x * uv1.y);
                float w1 = 1 - w2 - w3;
                int and6 = (1 << 6) - 1;
                float inv64 = 0.015625;		// 1/64
                float diff1 = ((blendData1 & and6) + 1) * inv64;
                float2 weight1 = float2(0.5 * (1 + diff1), 0.5 * (1 - diff1)) * w1;		//解码得到主权重+次权重
                ...
                half3 normal1 = SAMPLE_TEXTURE2D_ARRAY(_NormalArray, sampler_NormalArray, texUV, index1);
                half3 normal2 = SAMPLE_TEXTURE2D_ARRAY(_NormalArray, sampler_NormalArray, texUV, index2);
                half3 normal3 = SAMPLE_TEXTURE2D_ARRAY(_NormalArray, sampler_NormalArray, texUV, index3);
